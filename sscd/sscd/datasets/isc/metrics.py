...
def remove_duplicates(predictions: List[PredictedMatch]) -> List[PredictedMatch]:
    seen = set()
    unique_predictions = []
...
<<<<<<< HEAD
    ...
    for idx, p in enumerate(predictions):
        key = (p.query, p.db)
        if key in seen:
            duplicates.append((key, seen[key], idx))  # (중복된 key, 첫 인덱스, 현재 인덱스)
        else:
            seen[key] = idx

    if duplicates:
        print(f"❌ 중복된 (query, db) 쌍이 {len(duplicates)}개 발견되었습니다.")
        print("📌 중복 쌍 목록 (최대 10개 표시):")
        for i, (key, first_idx, dup_idx) in enumerate(duplicates[:10]):
            print(f"  [{i}] {key} - 처음 위치: {first_idx}, 중복 위치: {dup_idx}")
