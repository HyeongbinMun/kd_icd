...
class DISCEvalDataset:
    ...
    def __init__(
        ...
        gt_path = os.path.join(path, "dev_queries_groundtruth.csv")
...
class DISCTestDataset:
    ...
    def __init__(
        ...
        query_path = os.path.join(path, "dev_queries_test")
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            ...
            print(f"1차 이미지 로딩 실패: {filename}\n에러: {e}", flush=True)
            relative_path = os.path.relpath(filename, start='/hdd')
            backup_path = os.path.join('/hdd/disc21/backup', relative_path)

            try:
                img = default_loader(backup_path)
                print(f"백업 이미지로 대체 성공: {backup_path}", flush=True)
            except Exception as e2:
                print(f"백업 이미지 로딩 실패: {backup_path}\n에러: {e2}", flush=True)
                raise e2
